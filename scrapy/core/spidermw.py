...
class SpiderMiddlewareManager(MiddlewareManager):
        ...
        for method in self.methods["process_spider_input"]:
            try:
                ...
                if result is not None:
                    msg = (
                        f"Middleware {_fname(method)} must return None "
    ...
    def _evaluate_iterable(
        self, response, spider, iterable, exception_processor_index, recover_to
    ):
        try:
            for r in iterable:
                yield r
    ...
    def _process_spider_output(self, response, spider, result, start_index=0):
        ...
        for method_index, method in enumerate(method_list, start=start_index):
            ...
            if _isiterable(result):
                ...
                msg = (
                    f"Middleware {_fname(method)} must return an "
